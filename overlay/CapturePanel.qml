...
Item {
  ...
  readonly property string mediaScriptPath: {
    var u = Qt.resolvedUrl("cap-media.sh").toString()
    if (u.indexOf("file://") === 0) u = u.substring(7)
    return u
  }
  ...
  function detectClipboardImage() {
    imgProbe.command = ["sh", "-c", "wl-paste --list-types 2>/dev/null | grep -c '^image/' || true"]
    imgProbe.running = true
  }
  ...
  Process {
    ...
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: {
        root.hasImage = parseInt(String(text).trim()) > 0
      }
    }
  }
  ...
  Process {
    ...
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: {
        const imgPath = String(text).trim()
        if (imgPath.length > 0) {
          mediaProc.command = ["/bin/bash", root.mediaScriptPath, imgPath, "Quick capture image"]
          mediaProc.running = true
        } else {
          root.sending = false
          root.sendError = "no image in clipboard"
        }
      }
    }
  }
